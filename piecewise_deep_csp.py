...
#W = man_filter(x_train_filt, y_train,0.5)[:,0:5]
V = np.ones((50,1))
...
cost       = (layer2.negative_log_likelihood(y)[0]+.01*T.sum(T.pow(avg_v,2)),layer2.negative_log_likelihood(y)[1],\
              layer2.negative_log_likelihood(y)[2], layer2.negative_log_likelihood(y)[3],layer2.negative_log_likelihood(y)[4])
...
for i in range(epochs):
    for j in range(y_train.size/batch_size):
        (cost_ij,y_predij,inputij,Wij,bij) = train_model(j)

    #-np.mean(np.log(y_predij)[np.arange(y_train[j * batch_size: (j + 1) * batch_size,0].shape[0]), y_train[j * batch_size: (j + 1) * batch_size,0]])
    er = test_model()
    print 'Epoch = %i' % i
